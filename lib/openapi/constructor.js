...
module.exports = ({
  ...
  const amendModelReferences = (schema, cache = {}) => {
    if (schema) {
      const flat = flattenObject(schema);
      const refs = Object.keys(flat).filter((k) => k.indexOf('$ref') > -1);
      refs.forEach((ref) => {
        ...
        if (refValue.indexOf('#') === refValue.length - 1) {
          const schemaId = refValue.substring(0, refValue.indexOf('#'));
          const amended = cache[schemaId] || (cache[schemaId] = DEFAULTS.modelPath.concat(schemaId));
          set(
            schema,
            ref,
            amendModelReferences(amended, cache)
          );
        }
      });
    }
