  ...
  const resolveReferences = (schema, cache = {}) => {
    if (schema) {
      ...
      for (let i = 0; i < refs.length; i += 1) {
        const refValue = get(schema, refs[i]);
        if (refValue.indexOf('#') === refValue.length - 1) {
          const schemaId = refValue.substring(0, refValue.indexOf('#'));
          const amended =
            cache[schemaId] ||
            (cache[schemaId] = DEFAULTS.modelPath.concat(schemaId));
          set(schema, refs[i], resolveReferences(amended, cache));
        }
      }
    }
